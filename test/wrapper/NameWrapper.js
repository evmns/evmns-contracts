...
describe('Name Wrapper', () => {
  ...
  before(async () => {
    ...
    expect(await EnsRegistry.owner(namehash('evm'))).to.equal(
      BaseRegistrar.address,
    )
  })
  ...
  describe('wrap()', () => {
    ...
    it('Does not allow wrapping .evm 2LDs.', async () => {
      const label = 'wrapped'
      const labelHash = labelhash(label)
      await BaseRegistrar.register(labelHash, account, 1 * DAY)
      await EnsRegistry.setApprovalForAll(NameWrapper.address, true)
      await expect(
        NameWrapper.wrap(encodeName('wrapped.evm'), account2, EMPTY_ADDRESS),
    ...
    it('Rewrapping a previously wrapped unexpired name retains PCC', async () => {
      const label = 'test'
      const labelHash = labelhash(label)
      const wrappedTokenId = namehash(label + '.evm')
      const subLabel = 'sub'
      const subLabelHash = labelhash(subLabel)
      const subWrappedTokenId = namehash(`${subLabel}.${label}.evm`)
  ...
  describe('unwrap()', () => {
    ...
    it('Will not allow previous owner to unwrap name when name expires', async () => {
      ...
      await expect(
        ...
        `Unauthorised("${namehash('sub.unwrapped.evm')}", "${account}")`,
      )
    })
    ...
    it('Will not unwrap .evm 2LDs.', async () => {
      ...
      await expect(
        NameWrapper.unwrap(namehash('evm'), labelhash('unwrapped'), account),
      ).to.be.revertedWith('IncompatibleParent()')
    })
    ...
    it('Will not allow to unwrap with PCC/CU burned if expired', async () => {
      ...
      expect(await EnsRegistry.owner(namehash('sub.awesome.evm'))).to.equal(
        NameWrapper.address,
      )

      await expect(
        NameWrapper.unwrap(namehash('awesome.evm'), labelhash('sub'), account),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('sub.awesome.evm')}", "${account}")`,
      )
    })
    ...
    it('Will not allow to unwrap a name with the CANNOT_UNWRAP fuse burned if not expired', async () => {
      ...
      await expect(
        NameWrapper.unwrap(namehash('abc.evm'), labelhash('sub'), account),
      ).to.be.revertedWith(`OperationProhibited("${namehash('sub.abc.evm')}")`)
    })
  ...
  describe('wrapETH2LD()', () => {
    ...
    const labelHash = labelhash(label)
    const nameHash = namehash(label + '.evm')
    ...
    it('Cannot wrap a name if the owner has not authorised the wrapper with the .evm registrar.', async () => {
      await BaseRegistrar.register(labelHash, account, 1 * DAY)
    ...
    it('Can re-wrap a name that was wrapped has already expired on the .evm registrar', async () => {
      await BaseRegistrar.register(labelHash, account, 1 * DAY)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      await NameWrapper.wrapETH2LD(
        label,
      ...
      await expect(tx)
        .to.emit(NameWrapper, 'NameUnwrapped')
        .withArgs(namehash('wrapped2.evm'), EMPTY_ADDRESS)
      await expect(tx)
        .to.emit(NameWrapper, 'TransferSingle')
        .withArgs(account2, account, EMPTY_ADDRESS, nameHash, 1)
      await expect(tx)
        .to.emit(NameWrapper, 'NameWrapped')
        .withArgs(
          namehash('wrapped2.evm'),
          encodeName('wrapped2.evm'),
          account2,
    ...
    it('correctly reports fuses for a name that has expired and been rewrapped more permissively', async () => {
      ...
      ;[, fuses] = await NameWrapper.getData(namehash('sub.wrapped2.evm'))
      ...
      ;[, fuses, expiry] = await NameWrapper.getData(namehash('wrapped2.evm'))
    ...
    it('Allows an account approved by the owner on the .evm registrar to wrap a name.', async () => {
      await BaseRegistrar.register(labelHash, account, 1 * DAY)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
  ...
  describe('unwrapETH2LD()', () => {
    ...
    it('Allows the owner to unwrap a name.', async () => {
      ...
      expect(await NameWrapper.ownerOf(namehash('unwrapped.evm'))).to.equal(
        account,
      )
      await NameWrapper.unwrapETH2LD(labelHash, account, account)
      // transfers the controller on the .evm registrar to the target address.
      expect(await EnsRegistry.owner(namehash('unwrapped.evm'))).to.equal(
        account,
      )
      //Transfers the registrant on the .evm registrar to the target address
    ...
    it('emits Unwrap event', async () => {
      ...
      await expect(tx)
        .to.emit(NameWrapper, 'NameUnwrapped')
        .withArgs(namehash('unwrapped.evm'), account)
    })
    ...
    it('Does not allow a name to be unwrapped if CANNOT_UNWRAP fuse has been burned', async () => {
      ...
      await expect(
        ...
        'OperationProhibited("0x58077c1385b7b68ca1a53143bf46e14b9e1edbbfe3c9f1266f469fad4a0e6a90")',
      )
    })
    it('Unwrapping a previously wrapped unexpired name retains PCC and expiry', async () => {
      // register and wrap a name with PCC
  ...
  describe('approve()', () => {
    ...
    const subWrappedTokenId = namehash('sub.' + label + '.evm')
    let parentExpiry
    let result
    before(async () => {
      result = await ethers.provider.send('evm_snapshot')
    ...
    it('Does not allow approved accounts to extend expiry when expired', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash('sub.subdomain.evm')}")`,
      )
    })
    ...
    it('Approved address cannot unwrap .evm', async () => {
      await NameWrapper.approve(account2, wrappedTokenId)
      await expect(
    ...
    it('Approved address cannot unwrap non .evm', async () => {
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId,
        'sub',
        account,
      ...
      await NameWrapper.approve(account2, namehash('sub.subdomain.evm'))
      await expect(
        NameWrapper2.unwrap(wrappedTokenId, labelhash('sub'), account2),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('sub.subdomain.evm')}", "${account2}")`,
      )
    })
    ...
    it('Approval is cleared on unwrap()', async () => {
      ...
      await NameWrapper.approve(account2, namehash('sub.subdomain.evm'))
      expect(
        await NameWrapper.getApproved(namehash('sub.subdomain.evm')),
      ).to.equal(account2)
  ...
  describe('getApproved()', () => {
    ...
    it('Returns returns zero address when ownerOf() is zero', async () => {
      expect(await NameWrapper.ownerOf(namehash('unminted.evm'))).to.equal(
        EMPTY_ADDRESS,
      )
      expect(await NameWrapper.getApproved(namehash('unminted.evm'))).to.equal(
        EMPTY_ADDRESS,
      )
    })
  ...
  describe('upgrade()', () => {
    describe('.evm', () => {
      const encodedName = encodeName('wrapped2.evm')
      const label = 'wrapped2'
      const labelHash = labelhash(label)
      const nameHash = namehash(label + '.evm')

      it('Upgrades a .evm name if sender is owner', async () => {
        await BaseRegistrar.register(labelHash, account, 1 * DAY)
        const expectedExpiry = await BaseRegistrar.nameExpires(labelHash)
        await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      ...
      it('Upgrades a .evm name if sender is authorised by the owner', async () => {
        await BaseRegistrar.register(labelHash, account, 1 * DAY)
        const expectedExpiry = await BaseRegistrar.nameExpires(labelHash)
        await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
        await NameWrapper.setApprovalForAll(account2, true)
    ...
    describe('other', () => {
      ...
      const name = label + '.' + parentLabel + '.evm'
      const parentLabelHash = labelhash(parentLabel)
      const parentHash = namehash(parentLabel + '.evm')
  ...
  describe('setFuses()', () => {
    ...
    const wrappedTokenId = namehash('fuses.evm')
    ...
    it('cannot burn fuses as the previous owner of a .evm when the name has expired', async () => {
      await BaseRegistrar.register(labelhash('abc'), account, 1 * DAY)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      await NameWrapper.wrapETH2LD('abc', account, CANNOT_UNWRAP, EMPTY_ADDRESS)
      ...
      await expect(
        NameWrapper.setFuses(namehash('abc.evm'), CANNOT_UNWRAP),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('abc.evm')}", "${account}")`,
      )
    })

    it('cannot burn fuses as a previous owner of a non .evm when the name has expired', async () => {
      await BaseRegistrar.register(labelhash('abc'), account, 1 * DAY)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      await NameWrapper.wrapETH2LD('abc', account, CANNOT_UNWRAP, EMPTY_ADDRESS)
      ...
      await expect(
        NameWrapper.setFuses(namehash('sub.abc.evm'), CANNOT_UNWRAP),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('sub.abc.evm')}", "${account}")`,
      )
    })
    ...
    it('Will not allow burning fuses if PARENT_CANNOT_CONTROL has not been burned', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("0x79c55037bb3e95c009c177e1a8fa1d63b41e6e12ff0305dfe4209f8ca0a5d01c")`,
      )
    })
    ...
    it('Will not allow burning fuses of subdomains if CANNOT_UNWRAP has not been burned', async () => {
      ...
      await expect(
        NameWrapper.setFuses(namehash('sub.abc.evm'), CANNOT_TRANSFER),
      ).to.be.revertedWith(
        `OperationProhibited("0x79c55037bb3e95c009c177e1a8fa1d63b41e6e12ff0305dfe4209f8ca0a5d01c")`,
      )
    })

    it('Will not allow burning fuses of .evm names unless CANNOT_UNWRAP is also burned.', async () => {
      await BaseRegistrar.register(tokenId, account, 1 * DAY)
    ...
    it('can set fuses and burn transfer', async () => {
      ...
      const wrappedTokenId = namehash('fuses3.evm')
    ...
    it('can set fuses and burn canCreateSubdomains', async () => {
      ...
      expect(
        await EnsRegistry.owner(namehash('creatable.fuses2.evm')),
      ).to.equal(NameWrapper.address)

      expect(
        await NameWrapper.ownerOf(namehash('creatable.fuses2.evm')),
      ).to.equal(account)
      ...
      await expect(
        ...
        `OperationProhibited("${namehash('uncreatable.fuses2.evm')}")`,
      )
    })
  })
  ...
  describe('extendExpiry()', () => {
    ...
    it('Allows parent owner to set expiry without CAN_EXTEND_EXPIRY burned', async () => {
      ...
      ;[, fuses, expiry] = await NameWrapper.getData(namehash('sub.fuses.evm'))
    ...
    it('Does not allow owner of .evm 2LD to set expiry', async () => {
      await registerSetupAndWrapName('fuses', account, CANNOT_UNWRAP)

      let [, fuses, expiry] = await NameWrapper.getData(namehash('fuses.evm'))

      expect(fuses).to.equal(IS_DOT_ETH | PARENT_CANNOT_CONTROL | CANNOT_UNWRAP)

      await expect(
        NameWrapper.extendExpiry(namehash('evm'), tokenId, expiry),
      ).to.be.revertedWith(`OperationProhibited("${wrappedTokenId}")`)
    })
    ...
    it('Does not allow .evm 2LD owner to set expiry on child if the .evm 2LD is expired but grace period has not ended', async () => {
      await registerSetupAndWrapName('fuses', account, CANNOT_UNWRAP)
      const parentExpiry = await BaseRegistrar.nameExpires(tokenId)
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId,
    ...
    it('Allows child owner to set expiry if parent .evm 2LD is expired but grace period has not ended', async () => {
      await registerSetupAndWrapName('fuses', account, CANNOT_UNWRAP)
      const parentExpiry = await BaseRegistrar.nameExpires(tokenId)
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId,
    ...
    it('Does not allow extendExpiry() to be called on unwrapped names', async () => {
      ...
      let registryOwner = await EnsRegistry.owner(namehash('sub.fuses.evm'))
    ...
    it('Emits Expiry Extended event', async () => {
      ...
      await expect(tx)
        ...
        .withArgs(namehash(`sub.${label}.evm`), parentExpiry.add(GRACE_PERIOD))
    })
  })
  ...
  describe('setChildFuses()', () => {
    ...
    it('Does not allow parent owners to burn parent controlled fuses after burning PCC', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash('sub.fuses.evm')}")`,
      )
    })
    ...
    it('Does not allow non-parent owners to set child fuses', async () => {
      const subWrappedTokenId = namehash('sub.fuses.evm')
      await registerSetupAndWrapName('fuses', account, CANNOT_UNWRAP)
      await NameWrapper.setSubnodeOwner(wrappedTokenId, 'sub', account, 0, 0)
    ...
    it('should not allow .evm to call setChildFuses()', async () => {
      await registerSetupAndWrapName('fuses', account, 0)
      ...
      ).to.be.revertedWith(`Unauthorised("${namehash('evm')}", "${account}")`)
    })
  ...
  describe('setSubnodeOwner()', async () => {
    ...
    it('Can be called by the owner of a name and sets this contract as owner on the ENS registry.', async () => {
      ...
      expect(await EnsRegistry.owner(namehash(`sub.${label}.evm`))).to.equal(
        NameWrapper.address,
      )

      expect(await NameWrapper.ownerOf(namehash(`sub.${label}.evm`))).to.equal(
        account,
      )
    })
    it('Can be called by an account authorised by the owner.', async () => {
    ...
    it('Fuses cannot be burned if the name does not have PARENT_CANNOT_CONTROL burned', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash(`${label2}.${label}.evm`)}")`,
      )
    })
    ...
    it('Does not allow IS_DOT_ETH to be burned', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash(`sub.${label}.evm`)}")`,
      )
    })
    ...
    it('Unwrapping within an external contract does not create any state inconsistencies', async () => {
      ...
      // set self as sub.test.evm owner
      await NameWrapper.setSubnodeOwner(
        namehash('test.evm'),
        'sub',
        account,
    ...
    it('Expired subnames should still be protected by CANNOT_CREATE_SUBDOMAIN on the parent', async () => {
      ...
      const subWrappedTokenId = namehash(`${subLabel}.${label}.evm`)
      const subWrappedTokenId2 = namehash(`${subLabel2}.${label}.evm`)
      await registerSetupAndWrapName(label, account, CANNOT_UNWRAP)
      const parentExpiry = await BaseRegistrar.nameExpires(labelHash)
      // Confirm that the name is wrapped
      expect(await NameWrapper.ownerOf(wrappedTokenId)).to.equal(account)
  ...
  describe('setSubnodeRecord()', async () => {
    ...
    it('Fuses will remain 0 if expired', async () => {
      ...
      const [, fuses] = await NameWrapper.getData(namehash(`sub.${label}.evm`))
      expect(fuses).to.equal(0)
    })
    ...
    it('Sets the appropriate values on the ENS registry', async () => {
      ...
      const node = namehash(`sub.${label}.evm`)
  ...
  describe('setRecord()', () => {
    ...
    it('Setting the owner to 0 reverts if CANNOT_UNWRAP is burned', async () => {
      ...
      const wrappedTokenId2 = namehash('setrecord2.evm')
      const subWrappedTokenId = namehash('sub.setrecord2.evm')
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId2,
        'sub',
        account,
    ...
    it('Setting the owner to 0 on a .evm reverts', async () => {
      await registerSetupAndWrapName('setrecord2', account, 0)
      const wrappedTokenId2 = namehash('setrecord2.evm')
  ...
  describe('getData', () => {
    ...
    const subLabel = 'sub'
    const subLabelHash = labelhash(subLabel)
    const subNameHash = namehash(`${subLabel}.${label}.evm`)
    const subSubLabel = 'subsub'
    const subSubLabelhash = labelhash(subSubLabel)
    const subSubNameHash = namehash(`${subSubLabel}.${subLabel}.${label}.evm`)
  ...
  describe('registerAndWrapETH2LD()', () => {
    ...
    it('Does not allows fuse to be burned if CANNOT_UNWRAP has not been burned.', async () => {
      ...
      ).to.be.revertedWith(`OperationProhibited("${namehash(label + '.evm')}")`)
    })
  ...
  describe('isWrapped(bytes32 node)', () => {
    ...
    it('identifies a wrapped .evm name', async () => {
      expect(await NameWrapper['isWrapped(bytes32)'](wrappedTokenId)).to.equal(
        true,
      )
    })

    it('identifies an expired .evm name as unwrapped', async () => {
      const label = 'expired'
      const wrappedTokenId = namehash(label + '.evm')
    ...
    it('identifies an eth name registered on old controller as unwrapped', async () => {
      ...
      const labelHash = labelhash(label)
      const tokenId = namehash('oldcontroller.evm')
      await BaseRegistrar.register(labelHash, account, 1 * DAY)
    ...
    it('identifies an unregistered .evm name as unwrapped', async () => {
      const tokenId = namehash('abcdefghijklmnop.evm')
      expect(await NameWrapper['isWrapped(bytes32)'](tokenId)).to.equal(false)
    })
    ...
    it('identifies a wrapped subname', async () => {
      await NameWrapper.setSubnodeOwner(wrappedTokenId, 'sub', account, 0, 0)
      const subTokenId = namehash('sub.something.evm')
  ...
  describe('NameWrapper.names preimage dictionary', () => {
    it('Does not allow manipulating the preimage db by manually setting owner as NameWrapper', async () => {
      ...
      const sub1Domain = sub1Label + '.' + label + '.evm' // sub1.base.evm
      ...
      const sub2Domain = sub2Label + '.' + sub1Domain // sub2.sub1.base.evm
      ...
      const sub3Domain = sub3Label + '.' + sub2Domain // eth.sub2.sub1.base.evm
  ...
  describe('Grace period tests', () => {
    ...
    const wrappedTokenId = namehash(label + '.evm')
    const subLabel = 'sub'
    const subTokenId = namehash(subLabel + '.' + label + '.evm')
    let parentExpiry
    before(async () => {
      await registerSetupAndWrapName(label, account, CANNOT_UNWRAP)
      ;[, , parentExpiry] = await NameWrapper.getData(wrappedTokenId)
      ...
      // move .evm name to expired and be within grace period
      await evm.advanceTime(2 * DAY)
      await evm.mine()
    ...
    it('When a .evm name is in grace period it cannot call setSubnodeOwner', async () => {
      await expect(
        NameWrapper.setSubnodeOwner(
          wrappedTokenId,
          subLabel,
    ...
    it('When a .evm name is in grace period it cannot call setSubnodeRecord', async () => {
      await expect(
        NameWrapper.setSubnodeRecord(
          wrappedTokenId,
          subLabel,
    ...
    it('When a .evm name is in grace period it cannot call setRecord', async () => {
      await expect(
        NameWrapper.setRecord(wrappedTokenId, account2, EMPTY_ADDRESS, 0),
      ).to.be.revertedWith(`Unauthorised("${wrappedTokenId}", "${account}")`)
    })

    it('When a .evm name is in grace period it cannot call safeTransferFrom', async () => {
      await expect(
        NameWrapper.safeTransferFrom(
          account,
          account2,
    ...
    it('When a .evm name is in grace period it cannot call batchSafeTransferFrom', async () => {
      await expect(
        NameWrapper.safeBatchTransferFrom(
          account,
          account2,
    ...
    it('When a .evm name is in grace period it cannot call setResolver', async () => {
      await expect(
        NameWrapper.setResolver(wrappedTokenId, EMPTY_ADDRESS),
      ).to.be.revertedWith(`Unauthorised("${wrappedTokenId}", "${account}")`)
    })

    it('When a .evm name is in grace period it cannot call setTTL', async () => {
      await expect(NameWrapper.setTTL(wrappedTokenId, 0)).to.be.revertedWith(
        `Unauthorised("${wrappedTokenId}", "${account}")`,
      )
    })

    it('When a .evm name is in grace period it cannot call setFuses', async () => {
      await expect(NameWrapper.setFuses(wrappedTokenId, 0)).to.be.revertedWith(
        `Unauthorised("${wrappedTokenId}", "${account}")`,
      )
    })

    it('When a .evm name is in grace period it cannot call setChildFuses', async () => {
      await expect(
        NameWrapper.setChildFuses(wrappedTokenId, labelhash('sub'), 0, 0),
      ).to.be.revertedWith(`Unauthorised("${wrappedTokenId}", "${account}")`)
    })

    it('When a .evm name is in grace period, unexpired subdomains can call setFuses', async () => {
      await NameWrapper2.setFuses(subTokenId, CANNOT_UNWRAP)
      const [, fuses] = await NameWrapper.getData(subTokenId)
      expect(fuses).to.equal(PARENT_CANNOT_CONTROL | CANNOT_UNWRAP)
    })

    it('When a .evm name is in grace period, unexpired subdomains can transfer', async () => {
    ...
    it('When a .evm name is in grace period, unexpired subdomains can set resolver', async () => {
      await NameWrapper2.setResolver(subTokenId, account)
      expect(await EnsRegistry.resolver(subTokenId)).to.equal(account)
    })

    it('When a .evm name is in grace period, unexpired subdomains can set ttl', async () => {
      await NameWrapper2.setTTL(subTokenId, 100)
      expect(await EnsRegistry.ttl(subTokenId)).to.equal(100)
    })

    it('When a .evm name is in grace period, unexpired subdomains can call setRecord', async () => {
    ...
    it('When a .evm name is in grace period, unexpired subdomains can call setSubnodeOwner', async () => {
      await NameWrapper2.setSubnodeOwner(subTokenId, 'sub2', account2, 0, 0)
      expect(await NameWrapper.ownerOf(namehash('sub2.sub.test.evm'))).to.equal(
        account2,
      )
    })

    it('When a .evm name is in grace period, unexpired subdomains can call setSubnodeRecord', async () => {
      await NameWrapper2.setSubnodeRecord(
        subTokenId,
        'sub2',
        account2,
    ...
    it('When a .evm name is in grace period, unexpired subdomains can call setChildFuses if the subdomain exists', async () => {
      await NameWrapper2.setSubnodeOwner(subTokenId, 'sub2', account2, 0, 0)
      await NameWrapper2.setChildFuses(subTokenId, labelhash('sub2'), 0, 100)
      const [owner, fuses, expiry] = await NameWrapper.getData(
        namehash('sub2.sub.test.evm'),
      )
      expect(owner).to.equal(account2)
      expect(expiry).to.equal(100)
      expect(fuses).to.equal(0)
  ...
  describe('Registrar tests', () => {
    ...
    const wrappedTokenId1 = namehash('sub1.evm')
    ...
    it('Reverts when attempting to call token owner protected function on an unwrapped name', async () => {
      ...
      // XXX: note that at this step, the hackler should use the current .evm
      // registrar to directly register `sub1.evm` to himself, without wrapping
      // the name.
      ...
      //
      // XXX: corrently, `sub1.evm` becomes a normal node
      await EnsRegistryH.setOwner(wrappedTokenId1, NameWrapper.address)

      // create `sub2.sub1.evm` to the victim user with `PARENT_CANNOT_CONTROL`
      // burnt.
      await expect(
  ...
  describe('ERC1155 additional tests', () => {
    ...
    it('Approval on the Wrapper does not give permission to wrap the .evm name', async () => {
    ...
    it('Approval on the Wrapper does not give permission to wrap a non .evm name', async () => {
    ...
    it('When .evm name expires, it is untransferrable', async () => {
      await BaseRegistrar.register(labelhash(label), account, 1 * DAY)
      await NameWrapper.wrapETH2LD(label, account, 0, EMPTY_ADDRESS)
  ...
  describe('Implicit Unwrap tests', () => {
    ...
    const wrappedTokenId2 = namehash('sub2.sub1.evm')
    ...
    it('Trying to burn child fuses when re-registering a name on the old controller reverts', async () => {
      ...
      // create `sub2.sub1.evm` w/o fuses burnt
      await NameWrapperH.setSubnodeOwner(
        wrappedTokenId1,
        label2,
        hacker,
      ...
      // XXX: PREPARE HACK!
      // set `EnsRegistry.owner` of `sub1.evm` as the hacker himself.
      ...
      // set `sub2.sub1.evm` to the victim user w fuses burnt
    ...
    it('Renewing a wrapped, but expired name .evm in the wrapper, but unexpired on the registrar resyncs expiry', async () => {
      await NameWrapper.registerAndWrapETH2LD(
        label1,
        account,
        1 * DAY,
