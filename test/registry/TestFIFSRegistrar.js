...
contract('FIFSRegistrar', function (accounts) {
  ...
  it('should allow registration of names', async () => {
    await registrar.register(sha3('evm'), accounts[0], { from: accounts[0] })
    assert.equal(await ens.owner('0x0'), registrar.address)
    assert.equal(await ens.owner(namehash.hash('evm')), accounts[0])
  })
  ...
  describe('transferring names', async () => {
    ...
    it('should allow transferring name to your own', async () => {
      await registrar.register(sha3('evm'), accounts[1], { from: accounts[0] })
      assert.equal(await ens.owner(namehash.hash('evm')), accounts[1])
    })

    it('forbids transferring the name you do not own', async () => {
      await exceptions.expectFailure(
        registrar.register(sha3('evm'), accounts[1], { from: accounts[1] }),
      )
    })
  })
})
