...
contract('BaseRegistrar', function (accounts) {
  ...
  it('should allow registrations without updating the registry', async () => {
    ...
    var block = await web3.eth.getBlock(tx.receipt.blockHash)
    assert.equal(await ens.owner(namehash.hash('silentname.evm')), ZERO_ADDRESS)
  ...
  it('should permit the owner to reclaim a name', async () => {
    await ens.setSubnodeOwner(ZERO_HASH, sha3('evm'), accounts[0])
    await ens.setSubnodeOwner(
      namehash.hash('evm'),
      sha3('newname'),
      ZERO_ADDRESS,
    )
    assert.equal(await ens.owner(namehash.hash('newname.evm')), ZERO_ADDRESS)
    await ens.setSubnodeOwner(ZERO_HASH, sha3('evm'), registrar.address)
  ...
  it('should allow the owner to set a resolver address', async () => {
    ...
    assert.equal(await ens.resolver(namehash.hash('evm')), accounts[1])
  })
})
