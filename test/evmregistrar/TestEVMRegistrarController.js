...
contract('EVMRegistrarController', function () {
  let ens
  let resolver
  ...
  before(async () => {
    ...
    await ens.setSubnodeOwner(EMPTY_BYTES, sha3('evm'), baseRegistrar.address)

    const dummyOracle = await deploy('DummyOracle', '100000000', ownerAccount)
    ...
    const commitment = await controller2.makeCommitment(
      ...
      [
        ...
        resolver.interface.encodeFunctionData(
          ...
          [namehash('other.evm'), 'url', 'ethereum.com'],
        ),
      ],
      false,
      0,
  ...
  it('should permit a registration with resolver but no records', async () => {
    ...
    const nodehash = namehash('newconfigname2.evm')
    expect(await ens.resolver(nodehash)).to.equal(resolver.address)
  ...
  it('should allow anyone to renew a name without changing fuse expiry', async () => {
    ...
    var nodehash = namehash('newname.evm')
  ...
  it('non wrapped names can renew', async () => {
    ...
    const nodehash = namehash(`${label}.evm`)
