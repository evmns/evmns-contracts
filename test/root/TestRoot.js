...
contract('Root', function (accounts) {
  ...
  beforeEach(async function () {
    ...
    await ens.setSubnodeOwner('0x0', '0x' + sha3('evm'), root.address, {
      from: accounts[0],
    })
  ...
  describe('setSubnodeOwner', async () => {
    it('should allow controllers to set subnodes', async () => {
      await root.setSubnodeOwner('0x' + sha3('evm'), accounts[1], {
        from: accounts[0],
      })
    ...
    it('should fail when non-controller tries to set subnode', async () => {
      await exceptions.expectFailure(
        root.setSubnodeOwner('0x' + sha3('evm'), accounts[1], {
          from: accounts[1],
        }),
      )
    })
