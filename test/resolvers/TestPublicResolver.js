...
contract('PublicResolver', function (accounts) {
  ...
  beforeEach(async () => {
    ...
    await ens.setSubnodeOwner('0x0', sha3('evm'), accounts[0], {
      from: accounts[0],
    })
  })
  ...
  describe('dns', async () => {
    const basicSetDNSRecords = async () => {
      // a.evm. 3600 IN A 1.2.3.4
      const arec = '01610365766d000001000100000e10000401020304'
      // b.evm. 3600 IN A 2.3.4.5
      const b1rec = '01620365766d000001000100000e10000402030405'
      // b.evm. 3600 IN A 3.4.5.6
      const b2rec = '01620365766d000001000100000e10000403040506'
      // eth. 86400 IN SOA ns1.evmdns.xyz. hostmaster.test.evm. 2018061501 15620 1800 1814400 14400
      const soarec =
        '0365766d000006000100015180003a036e733106657468646e730378797a000a686f73746d61737465720574657374310365766d0078492cbd00003d0400000708001baf8000003840'
      const rec = '0x' + arec + b1rec + b2rec + soarec

      await resolver.setDNSRecords(node, rec, { from: accounts[0] })

      assert.equal(
        await resolver.dnsRecord(node, sha3(dnsName('a.evm.')), 1),
        '0x01610365766d000001000100000e10000401020304',
      )
      assert.equal(
        await resolver.dnsRecord(node, sha3(dnsName('b.evm.')), 1),
        '0x01620365766d000001000100000e1000040203040501620365766d000001000100000e10000403040506',
      )
      assert.equal(
        await resolver.dnsRecord(node, sha3(dnsName('evm.')), 6),
        '0x0365766d000006000100015180003a036e733106657468646e730378797a000a686f73746d61737465720574657374310365766d0078492cbd00003d0400000708001baf8000003840',
      )
    }
    it('permits setting name by owner', basicSetDNSRecords)

    it('should update existing records', async () => {
      // a.evm. 3600 IN A 4.5.6.7
      const arec = '01610365766d000001000100000e10000404050607'
      // eth. 86400 IN SOA ns1.evmdns.xyz. hostmaster.test.evm. 2018061502 15620 1800 1814400 14400
      const soarec =
        '0365766d000006000100015180003a036e733106657468646e730378797a000a686f73746d61737465720574657374310365766d0078492cbe00003d0400000708001baf8000003840'
      ...
      assert.equal(
        await resolver.dnsRecord(node, sha3(dnsName('a.evm.')), 1),
        '0x01610365766d000001000100000e10000404050607',
      )
      assert.equal(
        await resolver.dnsRecord(node, sha3(dnsName('evm.')), 6),
        '0x0365766d000006000100015180003a036e733106657468646e730378797a000a686f73746d61737465720574657374310365766d0078492cbe00003d0400000708001baf8000003840',
      )
    })

    it('should keep track of entries', async () => {
      // c.evm. 3600 IN A 1.2.3.4
      const crec = '01630365766d000001000100000e10000401020304'
      const rec = '0x' + crec
      ...
      hasEntries = await resolver.hasDNSRecords(node, sha3(dnsName('d.evm.')))
      assert.equal(hasEntries, false)

      // Update with no new data makes no difference
      await resolver.setDNSRecords(node, rec, { from: accounts[0] })
      hasEntries = await resolver.hasDNSRecords(node, sha3(dnsName('c.evm.')))
      assert.equal(hasEntries, true)

      // c.evm. 3600 IN A
      const crec2 = '01630365766d000001000100000e100000'
      const rec2 = '0x' + crec2
    ...
    it('should handle single-record updates', async () => {
      // e.evm. 3600 IN A 1.2.3.4
      const erec = '01650365766d000001000100000e10000401020304'
      const rec = '0x' + erec

      await resolver.setDNSRecords(node, rec, { from: accounts[0] })

      assert.equal(
        await resolver.dnsRecord(node, sha3(dnsName('e.evm.')), 1),
        '0x01650365766d000001000100000e10000401020304',
      )
    })

    it('forbids setting DNS records by non-owners', async () => {
      // f.evm. 3600 IN A 1.2.3.4
      const frec = '01660365766d000001000100000e10000401020304'
      const rec = '0x' + frec
      await exceptions.expectFailure(
