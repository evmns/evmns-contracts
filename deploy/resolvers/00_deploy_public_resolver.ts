...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const controller = await ethers.getContract('EVMRegistrarController', owner)
  ...
  if ((await registry.owner(ethers.utils.namehash('resolver.evm'))) === owner) {
    const pr = (await ethers.getContract('PublicResolver')).connect(
      await ethers.getSigner(owner),
    )
    const resolverHash = ethers.utils.namehash('resolver.evm')
