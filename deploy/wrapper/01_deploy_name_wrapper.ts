...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const resolver = await providerWithEns.getResolver('evm')
  if (resolver === null) {
    console.log(
      'No resolver set for .evm; not setting interface for NameWrapper',
    )
    return
  }
  ...
  console.log(
    `Setting NameWrapper interface ID ${interfaceId} on .evm resolver (tx: ${tx3.hash})...`,
  )
  await tx3.wait()
}
